      ...
      >
        ...
        <div className="bg-white" style={{ padding: "1.375rem 1.5rem", borderBottom: "0.5px solid var(--color-border-tertiary)" }}>
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: "1.125rem" }}>
            ...
            <button type="button" onClick={closeModal} style={{ background: "var(--color-background-secondary)", border: "none", borderRadius: 8, width: 30, height: 30, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "var(--color-text-secondary)", flexShrink: 0, marginLeft: 12 }}>
          ...
          {step === 1 ? (
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {REPORT_TYPES.map((rt) => {
                ...
                return (
                  <button key={rt.id} type="button" className="type-opt" onClick={() => setReportType(rt.id)} style={{ display: "flex", alignItems: "center", gap: 14, padding: "16px", background: sel ? "#f0f7ff" : "var(--color-background-secondary)", border: `${sel ? 2 : 1}px solid ${sel ? "#185fa5" : "var(--color-border-tertiary)"}`, borderRadius: 12, cursor: "pointer", textAlign: "left", transition: "all .15s" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
              {TIMELINES.map(({ label, sub }) => {
                ...
                return (
                  <button key={label} type="button" className="tl-opt" onClick={() => setTimeline(label)} style={{ padding: "12px 12px", background: sel ? "#f0f7ff" : "var(--color-background-secondary)", border: `${sel ? 2 : 1}px solid ${sel ? "#185fa5" : "var(--color-border-tertiary)"}`, borderRadius: 10, cursor: "pointer", textAlign: "left", transition: "all .15s" }}>
        ...
        <div className="bg-white" style={{ padding: "1rem 1.5rem", borderTop: "0.5px solid var(--color-border-tertiary)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <button type="button" onClick={closeModal} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 13, color: "var(--color-text-secondary)", padding: "6px 0" }}>Cancel</button>
          <div style={{ display: "flex", gap: 8 }}>
            {step === 2 && (
              <button type="button" onClick={() => setStep(1)} style={{ background: "none", border: "0.5px solid var(--color-border-secondary)", borderRadius: 8, padding: "8px 14px", cursor: "pointer", fontSize: 13, color: "var(--color-text-secondary)", display: "flex", alignItems: "center", gap: 5 }}>
